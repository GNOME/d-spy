...
#define G_LOG_DOMAIN "dspy-pattern-spec"
...
G_DEFINE_BOXED_TYPE (DspyPatternSpec, dspy_pattern_spec, dspy_pattern_spec_ref, dspy_pattern_spec_unref)

/**
 * SECTION:dspy-pattern-spec
 * @title: DspyPatternSpec
...
DspyPatternSpec *
dspy_pattern_spec_new (const gchar *needle)
{
  DspyPatternSpec *self;
  const gchar *tmp;

  if (needle == NULL)
    needle = "";

  self = g_slice_new0 (DspyPatternSpec);
  self->ref_count = 1;
...
const gchar *
dspy_pattern_spec_get_text (DspyPatternSpec *self)
{
...
static void
dspy_pattern_spec_free (DspyPatternSpec *self)
{
  g_clear_pointer (&self->parts, g_strfreev);
  g_clear_pointer (&self->needle, g_free);
  g_slice_free (DspyPatternSpec, self);
}
...
gboolean
dspy_pattern_spec_match (DspyPatternSpec *self,
...
DspyPatternSpec *
dspy_pattern_spec_ref (DspyPatternSpec *self)
{
...
void
dspy_pattern_spec_unref (DspyPatternSpec *self)
{
  g_return_if_fail (self);
  g_return_if_fail (self->ref_count > 0);

  if (g_atomic_int_dec_and_test (&self->ref_count))
    dspy_pattern_spec_free (self);
}
